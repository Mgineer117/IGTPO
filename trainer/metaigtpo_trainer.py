...
class MetaIGTPOTrainer:
    def __init__(
        ...
        self.meta_timesteps = int(self.timesteps * 0.25)
        self.igtpo_timesteps = int(self.timesteps * 0.75)

        self.log_interval = log_interval
        self.prune_interval = int(self.igtpo_timesteps / self.num_vectors)
        self.trim_interval = int(self.igtpo_timesteps / (self.num_local_updates - 2))
    ...
    def train(self) -> dict[str, float]:
        ...
        # Train loop
        self.prune_idx = int(self.meta_timesteps / self.prune_interval)
        self.trim_idx = int(self.meta_timesteps / self.trim_interval)
        eval_idx = 0
        ...
        with tqdm(
            ...
            while pbar.n < self.timesteps + self.init_timesteps:
                ...
                if current_step <= int((self.timesteps + self.init_timesteps) / 2):
                    loss_dict, total_timesteps = self.meta_train(current_step)
                else:
                    loss_dict, total_timesteps = self.igtpo_train(current_step)
                ...
                # === EVALUATIONS === #
                if current_step >= self.eval_interval * eval_idx + self.init_timesteps:
                    ### Eval Loop
                    self.policy.eval()
                    eval_idx += 1
        ...
        self.logger.print(
            f"Total {self.policy.name} training time: {(time.time() - self.start_time) / 3600} hours"
        )

    def meta_train(self, current_step: int):
        # === Initial Iteration ===
        ...
        # Meta-critic update
        meta_critic_loss_dict, meta_mean_value = self.meta_critic.learn(meta_batch)
        ...
        for i in range(self.num_vectors):
            ...
            batch_i["rewards"] = self.intrinsic_rewards(batch_i, self.eigenvectors[i])
            policy = deepcopy(self.policy)
            critic = self.subtask_critics.critics[i]

            loss_dict, timesteps, update_time, new_policy, gradients, _ = policy.learn(
                critic, batch_i, "local"
            )
        ...
        for j in range(1, self.num_local_updates):
            for i in range(self.num_vectors):
                ...
                (
                    ...
                    gradients,
                    _,
                ) = policy.learn(critic, option_batch, "local")
                loss_dict[f"{policy.name}-local/analytics/task_rewards"] = np.mean(
                    task_batch["rewards"]
                )
        ...
        for i in range(self.num_vectors):
            ...
            for j in reversed(range(self.num_local_updates - 1)):
                ...
                gradients = tuple(
                    g - self.policy.igtpo_actor_lr * h for g, h in zip(gradients, Hv)
                )
            meta_gradients.append(gradients)

        # Average across vectors
        meta_gradients_transposed = list(zip(*meta_gradients))  # Group by parameter
        ...
        loss_dict[f"{self.policy.name}/analytics/sample_time"] = total_sample_time
        loss_dict[f"{self.policy.name}/analytics/update_time"] = total_update_time
        loss_dict[f"{self.policy.name}/analytics/remaining_time (hr)"] = (
            remaining_time / 3600
        )  # Convert to hours
        loss_dict[f"{self.policy.name}/parameters/num vectors"] = self.num_vectors
        loss_dict[f"{self.policy.name}/parameters/num_local_updates"] = (
            self.num_local_updates
        )
        loss_dict[f"{self.policy.name}/analytics/Backtrack_iter"] = backtrack_iter
        loss_dict[f"{self.policy.name}/analytics/Backtrack_success"] = backtrack_success
        loss_dict[f"{self.policy.name}/analytics/target_kl"] = self.policy.target_kl
    ...
    def igtpo_train(self, current_step):
        # === Initial Iteration ===
        total_timesteps, total_sample_time, total_update_time = 0, 0, 0
        ...
        for j in range(1, self.num_local_updates):
            for i in range(self.num_vectors):
                ...
                if prefix == "local":
                    ...
                    loss_dict[f"{policy.name}-{prefix}/analytics/task_rewards"] = (
        ...
        if self.num_vectors > 1:
            if (
                current_step
                > self.prune_interval * (self.prune_idx + 1) + self.init_timesteps
            ):
                self.prune_idx += 1
        ...
        if self.num_local_updates > 2:
            if current_step > self.trim_interval * self.trim_idx + self.init_timesteps:
